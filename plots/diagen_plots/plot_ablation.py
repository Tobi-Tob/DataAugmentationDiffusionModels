...
# method = 'baseline'  # 'noise', 'noise_llm', 'filter', 'noise_llm_filter'
...
if __name__ == "__main__":
    ...
    for dataset in datasets:
        method_means = np.array(get_means_for_method(dataset, method, "test", epc))  # epc is set in plot_lineplot.py
    ...
    if method == "noise_llm_filter":
        for dataset in datasets:
            method_means = np.array(get_means_for_method(dataset, "noise_llm", "test", epc))
    ...
    for dataset in datasets:
        method_means_compare = np.array(get_means_for_method(dataset, compare_against, "test", epc))
    ...
    if method == "noise_llm_filter":
        for dataset in datasets:
            dataset_hist_without_filter[dataset] = dataset_means_without_filter[dataset] - dataset_means_compare[
                dataset]
            # To avoid empty bars:
            dataset_hist_without_filter[dataset] = np.array(
                [0.0005 if abs(val) < 0.001 else val for val in dataset_hist_without_filter[dataset]])
    ...
    for offset, dataset, color in zip(arr, datasets, colors):
        if method == "noise_llm_filter":
            x_pos = x + offset
            bars.append(ax.bar(x_pos, dataset_hist_without_filter[dataset], bar_width, label=get_dataset_name(dataset),
                               color=color))
            ...
            for i in range(len(dataset_hist[dataset])):
                ax.annotate('', xy=(x_pos[i], dataset_hist[dataset][i]),
                            xytext=(x_pos[i], dataset_hist_without_filter[dataset][i]),
    ...
    plt.savefig(f'ablation_test_{method}_gg_{compare_against}.pdf', format='pdf')
    plt.show()
    """
